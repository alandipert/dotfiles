...
(defun dotspacemacs/layers ()
  "Configuration Layers declaration. You should not put any user code in this function besides modifying the variable
values."
  (setq-default
...
(defun alandipert/toggle-semicolon ()
  "Toggles existence of a semicolon at the end of the current line."
  (interactive)
  (save-excursion
    (end-of-line)
    (let* ((last-char (byte-to-string (char-after (1- (point))))))
...
(defvar-local *last-quit-attempt* nil)

(defun alandipert/maybe-quit ()
  "Needs to be called twice within 1 second in order to quit
emacs. Meant to be bound to s-q (command-q) on macOS."
  (interactive)
  (let* ((timeout-sec 1)
         (now (elt (current-time) 1)))
    (cond ((null *last-quit-attempt*)
           (setq *last-quit-attempt* now))
          ((>= (- now *last-quit-attempt*) timeout-sec)
           (progn (setq *last-quit-attempt* nil)
                  (message "Not quitting -- timed out")))
          (t (kill-emacs)))))
...
you should place your code here."
  (let* ((extensions! (lambda (mode exts)
    ...
    ;; org/achiever stuff
    (load-file "~/Dropbox/snippets/achiever.el")
    ...
    (evil-leader/set-key "o+" 'alandipert/increase-font-size)
    (evil-leader/set-key "o-" 'alandipert/decrease-font-size)
    (evil-leader/set-key "or" 'redraw-display)
    (evil-leader/set-key "ot" 'org-todo)
    (evil-leader/set-key "oa" 'achiever-achieve)
    (evil-leader/set-key "ol"  'evil-lisp-state)
    (evil-leader/set-key "o;" 'alandipert/toggle-semicolon)

    ;; On Macs, Command-q should only kill emacs if it's pressed twice within a
    ;; second.
    (when (eq system-type 'darwin)
      (global-set-key (kbd "s-q") 'alandipert/maybe-quit))
